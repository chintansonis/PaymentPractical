...
/**
 * this pattern work on preventing user to enter invalid past year for expiry date. For instance, user can not add
 * year as 2021. It restrict user to type 2021 fully.
 */
class CreditCardExpiryInputFilter : InputFilter {

    private var currentYearLastTwoDigits: String =
    ...
    override fun filter(
        ...
        //do not insert if length is already 7
        if (dest.toString().length == 7) return ""
        ...
        if (dstart == 0) {
            //first month digit
            if (inputChar > '1') return "0$inputChar"
        }
        ...
        if (dstart == 6) {
            val inputYear = "" + dest.substring(dest.length - 3) + source.toString()
